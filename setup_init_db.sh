...
echo "INSERT INTO web_users (rollnumber, username, password, usertype, doamin_1, domain_2, domain_3, allocated_mentor) Values ("none", "core", "1234567890", "core", "none", "none", "none", "none");" >> db/init.sql
...
while read line; do
	...
	echo "INSERT INTO ${mentee}_task_submitted (Task_number, Sysad, Web, App) Values (3, "n", "n", "n");" >> db/init.sql
 	echo "INSERT INTO web_users (rollnumber, username, password, usertype, doamin_1, domain_2, domain_3, allocated_mentor) Values ($rollnumber, $mentee, "1234567890", "mentee", "none", "none", "none", "none");" >> db/init.sql
...
while read line; do
	...
	dom=$(echo "$line" | awk '{print $2}')
 	cap=$(echo "$line" | awk '{print $3}')
  	cap="${cap//[$'\n\r\t ']/}"
	echo "INSERT INTO web_users (rollnumber, username, password, usertype, doamin_1, domain_2, domain_3, allocated_mentor) Values ($cap, $mentor, "1234567890", "mentor", $dom, "none", "none", "none");" >> db/init.sql
